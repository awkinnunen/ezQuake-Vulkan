...
int FriendsPlatform_Registered(const char *base){char expected[2300],current[2300];return Command(base,expected,sizeof(expected))&&ReadCommand(current,sizeof(current))&&!_stricmp(expected,current);}
int FriendsPlatform_Register(const char *base,int enable){
 char command[2300];HKEY key,open;LONG e;
 if(!enable){if(!FriendsPlatform_Registered(base))return 0;e=RegDeleteTreeA(HKEY_CURRENT_USER,registration);if(e==ERROR_SUCCESS)SHChangeNotify(SHCNE_ASSOCCHANGED,SHCNF_IDLIST,NULL,NULL);return e==ERROR_SUCCESS;}
...
void FriendsPlatform_Init(const char *base){char name[160];if(PipeName(base,name,sizeof(name)))inbox=CreateMailslotA(name,512,0,NULL);}
...
void FriendsPlatform_Close(void){if(inbox!=INVALID_HANDLE_VALUE)CloseHandle(inbox);inbox=INVALID_HANDLE_VALUE;}
